    ...
    const fetchProfileData = async () => {
        try {
            ...
            const profileResponse = await axios.get(
                `https://recruitments-portal-backend.vercel.app/admin/profile/${email}`,
                {
                    headers: {
    ...
    const fetchDomainResponses = async (domain: string) => {
        try {
            ...
            const responseResponse = await axios.get(
                `https://recruitments-portal-backend.vercel.app/response/${email}`,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                    },
            ...
            {profileData && (
                <div className="flex w-[80%]">
                    <div className="bg-white p-6 rounded-md shadow-md w-1/2 mr-4">
                        ...
                        <div className="mb-2">
                            <span className="font-semibold">Email ID:</span> {profileData.EmailID}
                        </div>
                        <div className="mb-2">
                            <span className="font-semibold">Mobile No:</span> {profileData.Mobile_No}
                        </div>
                        <div className="mb-4">
                            <span className="font-semibold">Domains (Clicking on domain field will display the responses):</span>
                            <div className="ml-4">
                                <div>
                        ...
                        <div className="bg-white p-6 rounded-md shadow-md w-1/2 h-[70vh] overflow-y-auto">
                            ...
                            {domainResponses.map((response, index) => (
                                <div key={index} className="mb-4">
                                    ...
                                    <div className="mb-2">
                                        <span className="font-semibold">Domain:</span> {response.domain}
                                    </div>
                                    <div className="mb-2">
                                        <span className="font-semibold">Start Time:</span> {response.startTime}
                                    </div>
                                    <div className="mb-2">
                                        <span className="font-semibold">Submission Time:</span> {response.submissionTime}
                                    </div>
                                    <div className="mb-2">
                                        <span className="font-semibold">End Time:</span> {response.endTime}
                                    </div>
                                    <div>
                                        ...
                                        <div className="ml-4">
                                            {response.questions.map((question, questionIndex) => (
                                                <div key={questionIndex} className="mb-2">
                                                    ...
                                                    <div>{question.q}</div>
                                                    <div className="ml-4">
                                                        <span className="font-semibold">Answer:</span> {question.ans}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
